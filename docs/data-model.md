...
- 总 Token（v2，2026-09-16 起）= `input + output + reasoning + cache_read + cache_write`，含缓存读写，等价于 ccswitch 的「真实消耗 Tokens」；缓存命中率与缓存节省费用仍单独展示。v1（2026-09-15 之前）为 `input + output + reasoning`（不含缓存），跨版本数值不可比。

...
| 概览时长口径修复：卡片改「调用耗时累计」+墙钟对照、环比窗口可解释（午夜起点 → 昨日同时段）、`active_duration` 裁剪到 `[from,to)`、数据质量新增调用时长异常三档排查（OpenSpec `fix-overview-duration-semantics`） | ✅ 已交付：随 `c5fc540` 提交并推送（origin+github 同步、CI 由 push 触发）；lstable 镜像部署（healthy、0 告警、旧镜像保留 `<none>` 可回滚）；API+浏览器 UI 验收通过（卡片与「窗口 18 小时53分 · 含并行重叠」副文案、环比徽标、outlier 区块 0.0%/0 小时、控制台零错误，本地同版本实例挂载生产副本验收——生产为 OIDC-only 未改其认证）；OpenSpec 已按 §13 同步并归档 `archive/2026-09-24-fix-overview-duration-semantics` | 2026-09-24 |
| 调用时长采集根因修复：opencode 改用 per-message `time.created` 作计时起点（turn 起点退出计时 + 6h 回看上界）、claude 诚实置空、入库 24h 软降级 + `first_response_at` 协议校验、迁移 024 存量修复（fr 代理 256,644 行 + claude 5,364 行 + `>6h` 残差 16 行）与 settings 门控全量重建；全库时长 101,963h→1,949.3h（−98.1%）、`duration>6h=0`（OpenSpec `fix-duration-collection-origin`） | ✅ 已交付：随 `c5fc540` 提交推送；Hub 镜像（本地 `sha256:ee0c86685d28` 直传）与唯一节点 aitools（=本机）Agent 同批更新（备份 `metria.bak-20260924-duration`、重启后轮询 errors=0）；生产 024 执行 16s/0 告警、部署前后双库 15 项不变量全过（codex 存量 133,803 行逐行不变、usage 存量逐行不变、rollup=明细 403,498）、迁移漏行=0、窗口内旧 Agent 36 行虚增 3.03h 属已披露窗口风险；§12.2 副本双轨验证（SQL 1.5s、17/17 不变量）；OpenSpec 已同步 4 ADDED/11 场景并归档 `archive/2026-09-24-fix-duration-collection-origin`，`validate --all` 37/37、结构自检通过 | 2026-09-24 |
...
- 修复后核对（同日 Asia/Shanghai，Hub 对 ccswitch）：Codex 请求数 530 vs 529、fresh 输入
  ...
  `include-cache-in-total-tokens` 反转为含缓存，与 ccswitch 用量面板的消耗总量直接可比。）

### 总 Token 口径与 ccswitch 对齐记录（2026-09-16）

- 背景：与 ccswitch 用量面板对比时，两边头号指标不可比 —— ccswitch 用量面板的消耗总量含缓存
...
- 实现：前端收敛在 `format.js:sumTokens` 单点（11 个页面自动跟随）；后端改
  ...
  （`DailyUsageChart`、总览趋势，后者补齐原本缺失的推理与缓存写入）；总览卡片改名「Token 消耗」（不额外强调「真实」，其余指标同样是真实数据）。
...
- 追加对齐（同日）：主流把推理算进「输出」（OpenAI 的 `output_tokens` 含 `reasoning_tokens`、
  Anthropic 的 thinking 本就在 output 内、计费按输出价），故：
  1) 展示上「输出」= `output + reasoning`（含推理），堆叠图由五层改四层（输入/输出/缓存读取/
     缓存写入），推理以「其中推理」出现且不再单独成柱；报告 HTML/PDF/文本的明细行、构成图分段、
     邮件 Token 趋势图（`daily_token_chart` 四序列）同步。
  2) 计费上推理按输出价计费：未配置推理价时回退输出价（`metria-pricing`），显式推理价优先；
     线上无可回退规则（443 条目录规则的推理价本就等于输出价），历史费用不变。
  3) 指标名去掉「真实」：改为「Token 消耗」。
  存储层仍保持 `output` 不含推理，故推理单价可覆盖、流量估算仍不把推理换算为响应字节。

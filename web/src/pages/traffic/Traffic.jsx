...
import { fmtChange, changeTone, fmtBytes, fmtPct, fmtUsd } from '../../services/format'
...
export default function Traffic() {
  ...
  const previousOverview = useQuery(`traffic-overview-previous${q(previousParams)}`, () => api(`/overview${q(previousParams)}`), { enabled: Boolean(previousRange) })
  ...
  const rank = (arr) => (arr || []).map((m) => ({ id: m.dimension, name: nodeNames[m.dimension] || m.dimension, value: m.estimated_traffic_bytes ?? 0, cost: m.cost_micro_usd }))
  ...
  return (
    <>
      ...
      <div className="mt-4 grid grid-cols-12 gap-6">
        <MetricCard label="总估算流量" value={(coverage.estimated_calls ?? 0) > 0 ? fmtBytes(o.estimated_total_bytes) : '—'} {...compare(o.estimated_total_bytes, previous?.estimated_total_bytes)} sub="估算流量" hint={`估算区间 ${fmtBytes(o.traffic_lower_bound_bytes)} ~ ${fmtBytes(o.traffic_upper_bound_bytes)}`} />
        <MetricCard label="请求估算流量" value={(coverage.estimated_calls ?? 0) > 0 ? fmtBytes(o.estimated_request_bytes) : '—'} {...compare(o.estimated_request_bytes, previous?.estimated_request_bytes)} sub="估算" />
        <MetricCard label="响应估算流量" value={(coverage.estimated_calls ?? 0) > 0 ? fmtBytes(o.estimated_response_bytes) : '—'} {...compare(o.estimated_response_bytes, previous?.estimated_response_bytes)} sub="估算" />
        <MetricCard label="估算覆盖率" value={fmtPct(coverage.ratio)} {...compare(coverage.ratio, previous?.traffic_coverage?.ratio)} sub={`${coverage.estimated_calls ?? 0} / ${coverage.total_calls ?? 0} 次调用`} hint={`${coverage.unavailable_calls ?? 0} 次调用缺少足够数据`} />
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-gray-800 shadow-xs rounded-2xl border border-gray-200 dark:border-gray-700/60 p-6">
          ...
          <RankingList items={rank(byAgent.data?.by)} valueKey="value" labelKey="name" format={fmtBytes} secondaryKey="cost" secondaryFormat={fmtUsd} limit={5} onItemClick={(a) => navigate(`/agents/${encodeURIComponent(a.id)}`)} />
        </div>
        <div className="bg-white dark:bg-gray-800 shadow-xs rounded-2xl border border-gray-200 dark:border-gray-700/60 p-6">
          <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4">模型估算流量排行</h2>
          <RankingList items={rank(byModel.data?.by)} valueKey="value" labelKey="name" format={fmtBytes} secondaryKey="cost" secondaryFormat={fmtUsd} limit={5} onItemClick={(m) => navigate(`/models/${encodeURIComponent(m.id)}`)} />
        </div>
        <div className="bg-white dark:bg-gray-800 shadow-xs rounded-2xl border border-gray-200 dark:border-gray-700/60 p-6">
          <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4">节点估算流量排行</h2>
          <RankingList items={rank(byNode.data?.by)} valueKey="value" labelKey="name" format={fmtBytes} secondaryKey="cost" secondaryFormat={fmtUsd} limit={5} onItemClick={(n) => navigate(`/nodes/${encodeURIComponent(n.id)}`)} />
        </div>
      </div>
    </>
  )

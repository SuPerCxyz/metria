      ...
      {missingPricing.length > 0 && (
        <div className="mt-4 bg-white dark:bg-gray-800 shadow-xs rounded-2xl border border-gray-200 dark:border-gray-700/60 p-6">
          ...
          <SortableList
            ...
            renderItem={(m) => (
              <button key={`${m.provider}/${m.model}`} type="button" onClick={() => navigate(`/models/${encodeURIComponent(m.model)}`)} className="px-3 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-xs font-medium">
                {m.model} · {m.calls} 次
              </button>
            )}
          />
